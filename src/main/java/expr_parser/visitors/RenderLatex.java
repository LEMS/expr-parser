...
public class RenderLatex extends ARenderAs {
	...
	public String visitPow(LEMSExpressionParser.PowContext ctx) {
		List<Integer> needFlippedExponent = Arrays.asList(
				LEMSExpressionParser.SIN, LEMSExpressionParser.COS,
				LEMSExpressionParser.TAN, LEMSExpressionParser.SINH,
				LEMSExpressionParser.COSH, LEMSExpressionParser.TANH);
		...
		if (needFlippedExponent.contains(nextLeftToken.getType())) {
			...
			return left.replace(tokenText,
					tokenText + "^" + TextUtils.bracketize(right));
		} else {
	...
	String adaptBinOp(Token op, String left, String right) {
		...
		case LEMSExpressionParser.DIV:
			return "\\frac" + TextUtils.bracketize(left)
					+ TextUtils.bracketize(right);
		default:
