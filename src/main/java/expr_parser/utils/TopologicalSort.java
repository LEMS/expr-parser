...
public class TopologicalSort
{
	...
	public static <T> List<T> sort(DirectedGraph<T> graph)
	{
		...
		/*
		 * We'll also maintain a third set consisting of all nodes that have been fully expanded. If the graph contains a cycle, then we can detect this by noting that a node has been explored but not
		 * fully expanded.
		 */
	...
	/**
	 * Recursively performs a Depth-First Search from the specified node, marking all nodes encountered by the search.
	 *
	 * @param node
